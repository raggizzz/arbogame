...
import { User, Users, Mail, ArrowLeft, Cpu, Gamepad2 } from 'lucide-react';
...
const Login = () => {
  ...
  const handleAnonymousLogin = async () => {
    ...
    try {
      ...
      // Iniciar jogo com o modo selecionado
    ...
    <div className="min-h-screen bg-gradient-to-br from-dengue-blue via-blue-400 to-dengue-green flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[1, 2, 3, 4].map((num) => (
                <button
                  ...
                  className={`py-4 rounded-xl font-black text-2xl transition-all ${numPlayers === num
          ...
          {/* Modo de Jogo (Só aparece se numPlayers > 1) */}
          {numPlayers > 1 && (
            <div className="bg-gray-50 p-4 rounded-xl border-2 border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-3 rounded-lg flex flex-col items-center gap-2 transition-all ${!isLocalMultiplayer
                      ? 'bg-white border-2 border-primary-500 shadow-md'
                      : 'bg-transparent border-2 border-transparent hover:bg-gray-100'
                    }`}
                >
                  <Cpu className={`w-6 h-6 ${!isLocalMultiplayer ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className={`text-xs font-bold ${!isLocalMultiplayer ? 'text-primary-600' : 'text-gray-500'}`}>
                    VS CPU
                  </span>
                </button>
                ...
                >
                  <Gamepad2 className={`w-6 h-6 ${isLocalMultiplayer ? 'text-primary-500' : 'text-gray-400'}`} />
                  <span className={`text-xs font-bold ${isLocalMultiplayer ? 'text-primary-600' : 'text-gray-500'}`}>
                    LOCAL
                  </span>
                </button>
              ...
              <p className="text-xs text-gray-500 mt-3 text-center">
                {isLocalMultiplayer
                  ? '👥 Jogue com amigos no mesmo dispositivo'
                  : '🤖 O computador controla os outros jogadores'}
              </p>
            </div>
          )}
