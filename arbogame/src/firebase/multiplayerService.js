...
const PLAYER_COLORS = ['#00E65C', '#F59E0B', '#0BA5E9', '#EF4444'];

const createInitialGameState = (roomPlayers) => ({
  players: roomPlayers.map((player, index) => ({
    id: player.id,
    name: player.name,
    color: PLAYER_COLORS[index] || player.color,
    isAI: false,
    score: 0,
    skipTurn: false,
  ...
  try {
    ...
    return await runTransaction(db, async (transaction) => {
      ...
      if (roomData.status !== 'waiting') {
        throw new Error('Sala já iniciada');
      }

      if (roomData.players.some(p => p.id === playerId)) {
        return { roomId: freshRoom.id, ...roomData };
      }
      ...
      const availableColor = PLAYER_COLORS.find(c => !usedColors.includes(c)) || PLAYER_COLORS[roomData.players.length] || PLAYER_COLORS[0];
      const newPlayer = {
        id: playerId,
        name: playerName,
        position: 0,
      ...
      const players = [...roomData.players, newPlayer];
      ...
      return { roomId: freshRoom.id, ...roomData, players, currentPlayers: players.length };
    });
  } catch (error) {
...
export const startGame = async (roomId, hostId) => {
  try {
    ...
    await updateDoc(roomRef, {
      status: 'playing',
      gameState: createInitialGameState(roomData.players),
      updatedAt: serverTimestamp()
    });
